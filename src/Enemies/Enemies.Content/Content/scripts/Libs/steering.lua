...
function steering.wander(agent, target, direction, radius, distance, jitter, deltaTime, maxSpeed)
	...
	local temp = vector:create( math.random(-1,1) * jitterTimeSlice, math.random(-1,1) * jitterTimeSlice  )

	print(string.format("first add: Target = ", target.X, " , ", target.Y, " Temp = ", temp.X, ", ", temp.Y))
